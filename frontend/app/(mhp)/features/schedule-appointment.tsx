...
export default function ScheduleAppointment() {
  ...
  const scheduleAppointment = () => {
    if (!selectedPatientId) {
      setStatusMessage('Please select a patient before scheduling an appointment.');
      return;
    }
    const formattedDate = date.toISOString().split('T')[0]; // YYYY-MM-DD
    const formattedTime = time.toTimeString().split(' ')[0].substring(0, 5); // HH:MM
    if (!formattedDate || !formattedTime || !location) {
      setStatusMessage('Please fill in the date, time, and location.');
      return;
    }
  ...
  return (
    <Container safeArea edges={['top', 'bottom']} style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
        ...
        <Text style={styles.subtitle}>Plan a follow-up session with patient</Text>
        ...
        <Card style={styles.card} variant="elevated">
          ...
          <View style={styles.fieldWrapper}>
            ...
            <Input placeholder="Clinic or location" value={location} onChangeText={setLocation} />
          </View>

          <Button variant="primary" size="lg" style={styles.button} onPress={scheduleAppointment}>
            Schedule (Static)
          </Button>
        </Card>
