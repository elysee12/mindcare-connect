...
export default function ReportsScreen() {
  ...
  const { data: reportData, isLoading } = useQuery({
    ...
    queryFn: () => api.reports(timeframe, user?.id?.toString()),
    staleTime: 1000 * 30,
    enabled: !!user?.id,
  });
  ...
  const { data: reportList = [], isLoading: isReportListLoading } = useQuery({
    ...
    queryFn: () => api.reports(undefined, user?.id?.toString()),
    staleTime: 1000 * 30,
    enabled: !!user?.id,
  });
        ...
        {isReportListLoading ? (
          ...
          reportList.map((report: any) => (
            <Card key={report.id} style={styles.reportCard} variant="outlined">
              ...
              <Text style={styles.reportPatient}>Patient: {report.patient?.fullName || report.patient?.full_name || `#${report.patientId}`}</Text>
              <Text style={styles.reportDetails} numberOfLines={2}>{report.details}</Text>
            </Card>
          ))
        ) : (
          <Text style={styles.infoText}>No reports found yet.</Text>
        )}
